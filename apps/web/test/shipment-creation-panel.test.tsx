...
describe('ShipmentCreationPanel', () => {
  ...
  it('links draft shipments to the shipment workspace instead of picking inline', () => {
    renderPanel();

    const openDraft = screen.getByRole('link', { name: 'Open shipment 801' });
    expect(openDraft).toHaveAttribute('href', '/shipments/801');
    expect(screen.queryByRole('button', { name: /Pick shipment/ })).not.toBeInTheDocument();
  });

  it('shows ship, deliver, invoice, and payment actions only for matching shipment and invoice states', () => {
    renderPanel();

    expect(screen.getByRole('link', { name: 'Open shipment 801' })).toBeInTheDocument();
